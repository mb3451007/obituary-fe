    ...
    return (
      <div className={"dropdown w-[292px] desktopUserAcc:w-full h-[48px] "}>
        ...
        >
          {data.map((item, index) => (
            <li key={item.id}>
              <Link href={item.url}>{item.place}</Link>
            </li>
          ))}
        </ul>
      </div>
      ...
      >
        ...
        <div className="flex gap-[26px]">
          ...
          <div className="flex desktopUserAcc:w-[282px] w-full flex-col mobileUserAcc:hidden">
            ...
            <div className="border-b-2 border-[#A1B1D425] focus:outline-none focus:border-blue-300 w-full text-[#3C3E41] py-1 text-[16px] bg-transparent mt-6">
              {"  "}
            </div>
          </div>
        </div>
